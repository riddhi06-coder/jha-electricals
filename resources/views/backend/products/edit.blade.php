                    ...
                    <div class="card-body">
                        ...
                        <div class="row g-3">    
                            ...
                            <div class="tab-content" id="wizard-tabContent">
                                ...
                                <form class="row g-3 needs-validation custom-input" novalidate action="{{ route('master-products.update', $details->id) }}" method="POST" enctype="multipart/form-data">
                                    @csrf
                                    @method('PUT')
                                    <!-- Category Dropdown -->
                                    <div class="col-6">
                                    ...
                                    <div class="col-6">
                                        <label class="form-label" for="sub_category_id">Sub Product Category <span class="txt-danger">*</span></label>
                                        <select class="form-control @error('sub_category_id') is-invalid @enderror" id="sub_category_id" name="sub_category_id" required>
                                            ...
                                            @foreach($subcategories as $subcategory)
                                                <option value="{{ $subcategory->id }}" {{ (old('sub_category_id', $details->sub_category_id) == $subcategory->id) ? 'selected' : '' }}>
                                    ...
                                    <div class="col-6">
                                        ...
                                        <input type="text" class="form-control @error('product_name') is-invalid @enderror" id="product_name" name="product_name" value="{{ old('product_name', $details->product_name) }}" placeholder="Enter Product Name" required>
                                        @error('product_name')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                    ...
                                    <!-- Image Preview -->
                                    <div class="col-12">
                                    <img id="imagePreview" src="{{ asset('uploads/products/' . $details->image) }}" 
                                    alt="Image Preview" class="img-thumbnail" width="100" height="100">
                                    </div>

                                    <!-- Form Actions -->
                                    <div class="col-12 text-end">
                                        <a href="{{ route('master-products.index') }}" class="btn btn-danger px-4">Cancel</a>
...
<script>
    $(document).ready(function () {
        // Auto-load subcategories if a category is already selected
        var selectedCategory = $('#category_id').val();
        var selectedSubCategory = "{{ $details->sub_category_id }}";
        ...
        $('#category_id').change(function () {
            ...
            if (category_id) {
                fetchSubcategories(category_id, null);
            } else {
                $('#sub_category_id').html('<option value="">Select Product Sub Category</option>');
            }
        });

        function fetchSubcategories(category_id, selectedSubCategory) {
            $.ajax({
